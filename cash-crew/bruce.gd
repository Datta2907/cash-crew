...
func _physics_process(delta: float) -> void:
	...
	if Input.is_action_just_pressed("p1_shoot") and can_shoot:
		shoot()
		did_shoot_this_frame = true

	if Input.is_action_pressed("p1_right"):
		velocity.x = sprintingVelocity if Input.is_action_pressed("p1_run") else walkingVelocity
		facing_right = true
	...
	if Input.is_action_pressed("p1_jump") and (is_on_floor() or jumpCount < 2):
	...
	if did_shoot_this_frame:
		...
		_set_anim("run" if Input.is_action_pressed("p1_run") else "Walk")
	else:
		_set_anim("Idle")
